...
int main() {
    ...
    std::println(stderr, "");
    {
        ...
        static_assert(number_of_transformers == 2z);
        {
            size_t i = 1;
        ...
        std::println(stderr, "D4 ⊍ M1 ⊍ M2 = S4:");
...

        {
            ...
            for (p::concepts::PermutationView_like_c auto &trans :
                ...
                std::println(stderr, "t{0}^-1 * D4 * t{0}:", i);
                print_elements(
        ...
        for (auto &t : vecs | std::ranges::views::join) {
            ...
            auto group = generate_and_print_group(new_generators);
            //if (group.size() == 8) {
            //    std::println(stderr, "found group with 8 elements");
            //    break;
            //}
        }
    }
